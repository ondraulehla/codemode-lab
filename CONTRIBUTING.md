...
3. **Billed tokens stay in their own column.** When Anthropic API counts land,
   report them as billed tokens. Never average a billed number with an estimate.
4. **One module counts bytes.** `packages/meter`. If you need a size somewhere
   else, import it.
5. **Print the probe arguments next to the result.** A generic query flatters a
   server by three orders of magnitude. The reader has to be able to judge the
   call.
6. **Never call a tool that spends someone's money.** `planProbes` skips tools
   whose name starts with a verb like `ask`, `generate` or `create`, and tools
   with `readOnlyHint: false`. Keep it that way.
7. **Date every table.** Live servers drift. A table without a measurement date
...
before the run. Two of the three current tasks predict a loss. If a losing task
unexpectedly wins, correct the prediction and say why. Do not delete the task.
...
A server that needs a credential never appears in the browser demo. A token in a
